...
int main(int argc, char *argv[]) {
...
#else
  font = XLoadQueryFont(
      dpy, "-*-helvetica-medium-r-normal--20-*-*-*-p-100-iso8859-1");
#endif
  ...
  geo_mask = XWMGeometry(dpy, DefaultScreen(dpy), user_geo, NULL /*def_geo*/,
                         DEFAULT_BDWIDTH, &xsh, &xsh.x, &xsh.y, &xsh.width,
                         &xsh.height, &gravity);
  ...
  mainW =
      XCreateSimpleWindow(dpy, DefaultRootWindow(dpy), xsh.x, xsh.y, xsh.width,
                          xsh.height, DEFAULT_BDWIDTH, fgpix, bgpix);

  /* set window manager properties */
  XSetStandardProperties(dpy, mainW, "xhello", "xhello", None, argv, argc,
                         &xsh);
  ...
  XftColorAllocValue(dpy, DefaultVisual(dpy, DefaultScreen(dpy)),
                     DefaultColormap(dpy, DefaultScreen(dpy)), &xrcolor,
                     &xftcolor);
  /* Xft highlight color */
  xrcolor.red = 0xafff;
    ...
    if (event.xany.window == mainW) {
      ...
      case Expose:
        if (event.xexpose.count == 0 || force_expose) {
          ...
          if (XGetGeometry(dpy, mainW, &wtmp, &itmp, &itmp, &w, &h, &utmp,
                           &utmp) == 0)
            break;
          ...
          if (highlight)
            XftDrawRect(xftdraw, &hlcolor, x, y - extents.height, wtext,
                        extents.height);
          XftDrawString8(xftdraw, &xftcolor, font, x, y, (XftChar8 *)text,
                         strlen(text));
#else
          x = (w - XTextWidth(font, text, strlen(text))) / 2;
          y = htext +
              (h - htext + font->max_bounds.ascent - font->max_bounds.descent) /
                  2;
